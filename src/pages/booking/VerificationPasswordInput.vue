...
export default defineComponent({
    ...
    methods: {
        ...
        async checkPassword() {
            const pinNumber = this.firstPin; // 현재 입력된 PIN 번호

            // 백엔드에서 욕하는 JSON 형식으로 데이터 전송
            const formData = {
                pinNumber // "pinNumber" : "123456"
            };

            // 로컬 스토리지에 저장된 JWT 토큰 가져오기
            // 백엔드에서 @AuthenticationPrincipal Member member 이거 통과하고 싶으면
            // postman으로 발급 받은 JWT 토큰 값을 문자열로 여기에 작성해주세요. 
