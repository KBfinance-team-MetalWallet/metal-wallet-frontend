	...
	<div :class="$style.div">
		...
		<div :class="$style.groupParent">
			...
			<div v-if="seatAvailabilityStore.historyList.length" :class="$style.groupContainer">
				<ShowTimeInfo v-for="(history, index) in seatAvailabilityStore.historyList"
					:key="`${history.time}-${history.actorNames.join(',')}-${index}`" :time="history.time"
					:names="history.actorNames" :rSeats="parseInt(
						...
						" :sSeats="parseInt(
							...
							" :aSeats="parseInt(
								...
								" :musicalId="musicalId" :scheduleId="history.scheduleId" />
			</div>
