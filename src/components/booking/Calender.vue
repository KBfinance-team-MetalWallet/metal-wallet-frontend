	...
	<div class="calendar-box">
		<div class="calendar-controls">
			<button class="chevron-icon" @click="prevMonth">
				<img class="fa-solid fa-chevron-left" src="@/assets/booking/arrow-left.png" alt="arrow-left" />
			</button>
			<span class="current-year">{{ currentYear }}년 {{ currentMonth + 1 }}월</span>
			<button class="chevron-icon" @click="nextMonth">
				<img class="fa-solid fa-chevron-right" src="@/assets/booking/arrow-right.png" alt="arrow-right" />
			</button>
		</div>
		...
		<div v-else class="calendar-grid">
			...
			<div class="calendar-day" v-for="(day, index) in daysGrid"
				:key="day.date ? formatDate(day.date) : `no-date-${index}`">
				<div class="daily-today-contents">
					<div @click="handleDateClick(day.date)" :class="{
