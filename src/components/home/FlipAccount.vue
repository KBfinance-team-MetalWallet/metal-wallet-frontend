		...
		>
			...
			>
				...
				<div class="card" :style="{ backgroundColor: account.color }">
					<div class="front">
						<!-- 계좌 정보 -->
						<div class="div1">출금계좌 : {{ account.number }}</div>
...
<style scoped>
	...
	.slide-up-enter-active {
		animation: slideUp 1s forwards; /* 슬라이드 업 애니메이션 실행 */
	}

	.slide-up-leave-active {
		animation: slideDown 1s forwards; /* 슬라이드 다운 애니메이션 실행 */
	}

	@keyframes slideUp {
		from {
			transform: translateY(-100%); /* 시작 위치: 카드 위에서 멀리 떨어짐 */
			opacity: 0; /* 투명하게 시작 */
		}
		to {
	...
	@keyframes slideDown {
		from {
			transform: translateY(0); /* 시작 위치: 원래 자리 */
			opacity: 1; /* 불투명하게 시작 */
		}
		to {
			transform: translateY(-100%); /* 최종 위치: 카드 위로 이동 */
			opacity: 0; /* 투명하게 */
		}
	}
	.my-account-text {
	...
	.card-container {
		...
		transition: transform 0.5s ease, opacity 0.5s ease; /* 카드 이동 및 투명도 변경에 트랜지션 적용 */
	}
